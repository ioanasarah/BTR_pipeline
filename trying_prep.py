...
def pooling(
    ...
    mz_axis = mz_recalibrated

    peak_idxs = [
        np.argmin(np.abs(mz_axis - peak))
        for peak in bins
    ]
    pooled_spectra = aligned_matrix[:, peak_idxs]
    pooled_mz = mz_axis[peak_idxs]

    # ensure dense 2D numpy array — sparse slices cause shape issues downstream
    if issparse(pooled_spectra):
        pooled_spectra = np.asarray(pooled_spectra.todense())
    else:
        pooled_spectra = np.asarray(pooled_spectra)

    print(f"Pooled spectra shape: {pooled_spectra.shape}")  # sanity check
...
# def append_matrix_peaks(
#         pooled_spectra: np.ndarray,
#         pooled_mz: np.ndarray,
#         raw_spectra,           # data.X — the full recalibrated spectra (pixels × all m/z)
#         full_mz_axis: np.ndarray,
#         matrix_peaks_df: pd.DataFrame,
#         n_peaks: int = 5,
#         tol: float = 0.1
# ) -> tuple:
# # take the top-N matrix peaks by ratio from the ranked dataframe
#     top_matrix = matrix_peaks_df.head(n_peaks)
#     matrix_mz_values = top_matrix["mz"].values  # shape (n_peaks,)

#     print(f"[append_matrix_peaks] Appending {len(matrix_mz_values)} matrix columns "
#           f"at m/z: {np.round(matrix_mz_values, 4)}")

#     # for each matrix m/z, find the closest column in the full (recalibrated) m/z axis
#     # and extract that column from the raw spectra — same logic as pooling()
#     matrix_cols = []
#     actual_matrix_mz = []
#     for mz_val in matrix_mz_values:
#         col_idx = np.argmin(np.abs(full_mz_axis - mz_val))
#         # extract the column: if sparse, convert to dense 1D array
#         if issparse(raw_spectra):
#             col = np.array(raw_spectra[:, col_idx].todense()).flatten()
#         else:
#             col = raw_spectra[:, col_idx]
#         matrix_cols.append(col.astype(np.float32))
#         actual_matrix_mz.append(full_mz_axis[col_idx])

#     # stack matrix columns into (n_pixels, n_peaks)
#     matrix_block = np.stack(matrix_cols, axis=1)

#     # concatenate: biological peaks first, matrix last
#     # this ordering matters — downstream code uses the last n_peaks columns to detect matrix
#     augmented_spectra = np.concatenate([pooled_spectra, matrix_block], axis=1)
#     augmented_mz = np.concatenate([pooled_mz, np.array(actual_matrix_mz)])

#     print(f"[append_matrix_peaks] Feature matrix shape: "
#           f"{pooled_spectra.shape} → {augmented_spectra.shape}")

#     return augmented_spectra, augmented_mz, len(matrix_mz_values)
...
def append_matrix_peaks(
    ...
    # guard: ensure pooled_spectra is 2D
    if isinstance(pooled_spectra, np.matrix):
        pooled_spectra = np.asarray(pooled_spectra)
    if pooled_spectra.ndim != 2:
        pooled_spectra = pooled_spectra.reshape(-1, pooled_spectra.shape[-1] if pooled_spectra.ndim > 1 else 1)

    top_matrix = matrix_peaks_df.head(n_peaks)
    matrix_mz_values = top_matrix["mz"].values
    ...
    for mz_val in matrix_mz_values:
        col_idx = np.argmin(np.abs(full_mz_axis - mz_val))
        if issparse(raw_spectra):
            col = np.asarray(raw_spectra[:, col_idx].todense()).flatten()  # ensure 1D
        else:
            col = np.asarray(raw_spectra[:, col_idx]).flatten()            # ensure 1D
        matrix_cols.append(col.astype(np.float32))
        actual_matrix_mz.append(full_mz_axis[col_idx])

    matrix_block = np.stack(matrix_cols, axis=1)  # (n_pixels, n_peaks)

    # ensure pooled_spectra is a plain numpy array before concat
    pooled_spectra = np.asarray(pooled_spectra)


        
    print(f"pooled_spectra type: {type(pooled_spectra)}, shape: {pooled_spectra.shape}, ndim: {pooled_spectra.ndim}")
    print(f"matrix_block type: {type(matrix_block)}, shape: {matrix_block.shape}, ndim: {matrix_block.ndim}")
...
def tic_normalization(filtered_spectra: np.ndarray, run_folder:str,
    ...
    normalised_matrix = (filtered_spectra / tic[:, np.newaxis]) * target # divide each spectrum by its TIC to normalize for differences in total intensity between spectra
...
def preprocess_single_sample(zarr_path: str, 
    ...
    n_peaks = 0
    if matrix_peaks_df is not None:
        pooled_spectra, pooled_mz, n_peaks = append_matrix_peaks(
            pooled_spectra,
            pooled_mz,
            data.X, 
            mz,
    ...
    return matrix_3d, filtered_mz, mz, n_peaks
...
def run_preprocessing(params, run_folder):
    ...
    # identify matrix peaks first if matrix zarr available
        ...
        for i, zarr_path in enumerate(sample_zarr_paths):
            ...
            matrix_3d, filtered_mz, full_mz, n_matrix_peaks = preprocess_single_sample(zarr_path, params, run_folder, matrix_peaks_df)
            sample_matrices.append(matrix_3d)
            sample_mz_lists.append(filtered_mz)
            full_mz_axes.append(full_mz)

        np.save(f"{run_folder}/n_matrix_peaks.npy", np.array([n_matrix_peaks]))
